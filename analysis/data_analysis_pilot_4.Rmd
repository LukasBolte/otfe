...
knit: (function(inputFile, encoding) {
    rmarkdown::render(inputFile,
                     encoding=encoding,
                     output_file='pilot_4_quality_subsample') })
...
csv_file <- "data/all_apps_wide_2025-02-10.csv"
df <- read_csv(csv_file) 
df <- df %>% filter(participant.finished == 1)

# Define the folder name variable
folder_name <- "output_2025-02-07-pilot_4"
...
df <- df %>%
  mutate(
    high_quality = ifelse(attention_check == "passed" & cq_1_mistakes == 0 & cq_2_mistakes == 0, 1, 0)
  )
